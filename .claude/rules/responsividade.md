...
⚠️ **A regra é só de CELULAR** (dono 26/08: "em ipads, tablets a tela pode girar,
a regra deve valer apenas para celulares"). `max-height: 500px` separa celular
deitado (390–440px de altura) de tablet (768px+), e `pointer: coarse` tira o
desktop, onde a janela não gira — nasce deitada, com teclado e mouse. ⚠️ **esse
corte é DO DONO, não conveniência do agente**: no meio do caminho ele foi
removido, por eu supor que era a causa de "continua rodando", e o dono corrigiu
na mesma hora. A regra vale para TODOS os usuários, sem ninguém configurar nada
no aparelho — "quero que seja uma função nativa do aplicativo". Rotas públicas `/verificar/*` ficam
fora (`TravaOrientacao` fica no ramo do app em `main.jsx`, acima do portão de
auth — login e boot também são o app).

⚠️ **O que NÃO dá para eliminar:** a animação de rotação do próprio iOS. Ela é do
sistema, roda antes de a página saber de qualquer coisa, e nenhum app web a
remove — quem trava isso de verdade é app nativo, pelo `Info.plist`. O que a
trava garante é que, terminada a animação, o app está em pé. E o preço
declarado: a técnica é reprovada pela ACT rule b33eff do W3C, que a mapeia como
falha do **WCAG 2.1 SC 1.3.4 Orientation (AA)** — cabe na exceção "orientação
essencial", e a decisão de assumir isso foi do dono, com o pedido repetido 3×.
