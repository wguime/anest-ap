...
seguindo `.claude/rules/prompting.md`. O usuário não precisa lembrar do template — esta skill
...
1. **Identificar o tipo** pelo pedido (não perguntar se for óbvio):
   - **Tarefa** — prompt que o usuário cola numa sessão nova do Claude Code
...
2. **Inferir os elementos do template antes de perguntar.** O contexto vive no repo: módulo
   afetado (CLAUDE.md/mapa de módulos), quem usa (anestesistas, admin, DPO), comando de
