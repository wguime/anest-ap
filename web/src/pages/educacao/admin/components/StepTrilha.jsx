...
export function StepTrilha({
  ...
  const validateForm = useCallback(() => {
    ...
    if (formData.visibilityMode === 'RESTRICTED' && formData.allowedUserTypes.length === 0) {
      errors.allowedUserTypes = 'Selecione pelo menos um tipo de usuário';
    }
  ...
  const handleCreate = useCallback(async () => {
    ...
    try {
      const created = await addTrilha({
        ...
        tiposUsuario: formData.tiposUsuario,
        ...
        effectiveVisibility: formData.visibilityMode === 'RESTRICTED' ? 'RESTRICTED' : 'PUBLIC',
        effectiveAllowedUserTypes: formData.visibilityMode === 'RESTRICTED' ? formData.allowedUserTypes : [],
        status: 'DRAFT',
  ...
  // Handler para toggle de allowedUserTypes
  const handleToggleAllowedType = useCallback((tipo) => {
    setFormData(prev => ({
      ...prev,
      allowedUserTypes: prev.allowedUserTypes.includes(tipo)
        ? prev.allowedUserTypes.filter(t => t !== tipo)
        : [...prev.allowedUserTypes, tipo],
    }));
  }, []);
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="font-semibold">1. Trilha de Aprendizagem</h3>
      ...
      {mode === 'create' && (
        <div className="space-y-4">
          ...
          <FormField label="Tipos de Usuário (público-alvo)">
          ...
          <FormField label="Visibilidade">
            <div className="flex gap-2 flex-wrap">
              <Button
                variant={formData.visibilityMode === 'PUBLIC' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => setFormData(prev => ({ ...prev, visibilityMode: 'PUBLIC' }))}
              >
                Público
              </Button>
              <Button
                variant={formData.visibilityMode === 'RESTRICTED' ? 'primary' : 'ghost'}
                size="sm"
                onClick={() => setFormData(prev => ({ ...prev, visibilityMode: 'RESTRICTED' }))}
              >
                Restrito
              </Button>
            </div>
          ...
          {formData.visibilityMode === 'RESTRICTED' && (
            <FormField 
              label="Tipos Permitidos" 
              error={formErrors.allowedUserTypes}
              description="Apenas estes tipos de usuário poderão ver esta trilha"
            >
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                {Object.entries(TIPOS_USUARIO).map(([key, { label }]) => (
                  <div
                    ...
                    className={cn(
                      "flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-colors",
                      formData.allowedUserTypes.includes(key)
                        ? "border-primary bg-primary/5"
                        : "border-border hover:bg-muted"
                    )}
                    onClick={() => handleToggleAllowedType(key)}
                  >
                    <Checkbox
                      checked={formData.allowedUserTypes.includes(key)}
                      onChange={() => {}}
                      compact
                    />
                    <span className="text-sm">{label}</span>
