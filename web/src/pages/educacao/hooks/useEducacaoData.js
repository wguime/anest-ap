...
export function useEducacaoData({ useMock: useMockParam = USE_MOCK, autoFetch = true } = {}) {
  ...
  const { trilhasSan, cursosSan, modulosSan, aulasSan } = useMemo(() => {
    ...
    const trilhasValidas = (trilhas || []).map(t => ({
      ...t,
      cursos: (t.cursos || []).filter(cursoId => cursosById.has(cursoId)),
    }));
  ...
  const fetchTrilhas = useCallback(async () => {
    ...
    try {
      ...
      setTrilhas(data.length > 0 ? data : mockTrilhas);
    } catch (err) {
      console.error('Erro ao buscar trilhas:', err);
      setTrilhas(mockTrilhas); // Fallback para mock
    }
  }, [useMock]);
  ...
  const fetchCursos = useCallback(async () => {
    ...
    try {
      ...
      if (error) throw new Error(error);
      setCursos(data.length > 0 ? data : mockCursos);
    } catch (err) {
      console.error('Erro ao buscar cursos:', err);
      setCursos(mockCursos); // Fallback para mock
    }
  }, [useMock]);
  ...
  const fetchModulosByCurso = useCallback(async (cursoId) => {
    ...
    try {
      ...
      setModulos(prev => {
        ...
        const incoming = (data.length > 0 ? data : mockModulos.filter(m => m.cursoId === cursoId)) || [];
  ...
  const fetchAllModulos = useCallback(async () => {
    ...
    try {
      ...
      setModulos(dedupeById(allModulos.length > 0 ? allModulos : mockModulos));
    } catch (err) {
      console.error('Erro ao buscar módulos:', err);
  ...
  const fetchAllAulas = useCallback(async () => {
    ...
    try {
      ...
      setAulas(dedupeById(allAulas.length > 0 ? allAulas : mockAulas));
    } catch (err) {
  ...
  const fetchAll = useCallback(async () => {
    ...
    try {
      // Buscar trilhas e cursos em paralelo
      await Promise.all([fetchTrilhas(), fetchCursos()]);

      // Depois buscar módulos (depende dos cursos)
      await fetchAllModulos();

      // Por fim buscar aulas (depende dos módulos)
      await fetchAllAulas();

      // Relações (para reuso real + árvore)
  ...
  }, [useMock, cursos, modulos, aulas, fetchTrilhas, fetchCursos, fetchAllModulos, fetchAllAulas, fetchAllRelacoes]);
  ...
  }, [autoFetch, useMock]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  }, [useMock, userId, fetchCursos, fetchAllModulos, fetchAllAulas]);
  ...
  const deleteModulo = useCallback(async (id) => {
      ...
      if (modulo?.cursoId) {
        setCursos(prev => prev.map(c =>
          c.id === modulo.cursoId
            ? {
              ...c,
              modulos: c.modulos?.filter(m => m.id !== id),
            }
            : c
        ));
      }
    ...
    try {
      ...
      if (success && modulo) {
        await fetchModulosByCurso(modulo.cursoId);
        await fetchAllAulas();
      }
    } catch (err) {
  ...
  }, [useMock, modulos, userId, fetchModulosByCurso, fetchAllAulas]);
  ...
  const contentTree = useMemo(() => {
    ...
    return (trilhasSan || []).map((trilha) => {
      const cursosOrdered = (trilha.cursos || [])
        .map((cursoId) => cursosById.get(cursoId))
        .filter(Boolean);
  ...
  }, [trilhasSan, cursosSan, modulosSan, aulasSan, getModulosByCursoId, getAulasByModuloId]);
