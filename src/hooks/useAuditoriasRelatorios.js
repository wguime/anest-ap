...
/**
 * Hook para gerenciar documentos de Relatorios de Auditorias
 * @param {string} tipo - Tipo de relatorio (consolidado_rops, higiene_maos, medicamentos, conformidade)
 * @returns {Object} Funcoes e estados do hook
 */
export function useAuditoriasRelatorios(tipo = null) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [relatorios, setRelatorios] = useState([]);
  const [counts, setCounts] = useState({ consolidado_rops: 0, higiene_maos: 0, medicamentos: 0, conformidade: 0, total: 0 });

  /**
   * Carrega relatorios de auditoria por tipo
  ...
  const loadRelatorios = useCallback(async (tipoParam) => {
    ...
    if (!tipoToLoad) {
      setError('Tipo de relatorio de auditoria nao especificado');
      return [];
    }

    const config = getAuditoriaRelatorioConfig(tipoToLoad);
    if (!config) {
      setError('Tipo de relatorio de auditoria invalido');
      return [];
    }
    ...
    try {
      ...
      console.error('Erro ao carregar relatorios de auditoria:', err);

      if (err.code === 'permission-denied') {
        setError('Sem permissao para acessar relatorios de auditoria. Faca login novamente.');
        ...
        console.warn('Aviso ao carregar relatorios de auditoria:', err.message);
        setRelatorios([]);
      }
  ...
  /**
   * Carrega todos os relatorios de auditorias de todos os tipos
  ...
  const loadAllRelatorios = useCallback(async () => {
    ...
    try {
      ...
      const tipos = ['consolidado_rops', 'higiene_maos', 'medicamentos', 'conformidade'];
      const allDocs = [];

      for (const t of tipos) {
        const config = getAuditoriaRelatorioConfig(t);
        if (config) {
          try {
            ...
            console.warn(`Erro ao carregar relatorios de auditoria ${t}:`, e);
          }
        }
      }
      ...
      console.error('Erro ao carregar todos os relatorios de auditoria:', err);
      setError('Erro ao carregar relatorios de auditoria');
      setLoading(false);
      return [];
    }
  }, []);
  ...
  /**
   * Carrega contagem de relatorios de auditoria por tipo
   */
  const loadCounts = useCallback(async () => {
    try {
      // Contar cada colecao
      const countData = { consolidado_rops: 0, higiene_maos: 0, medicamentos: 0, conformidade: 0, total: 0 };
      ...
      countData.total = countData.consolidado_rops + countData.higiene_maos + countData.medicamentos + countData.conformidade;
      setCounts(countData);
      return countData;
    } catch (err) {
      console.error('Erro ao carregar contagens:', err);
      return { consolidado_rops: 0, higiene_maos: 0, medicamentos: 0, conformidade: 0, total: 0 };
    }
  }, []);

  /**
   * Faz upload de um novo relatorio de auditoria
  ...
  const uploadRelatorio = useCallback(async (tipoParam, file, metadata, user) => {
    ...
    if (!config) {
      throw new Error('Tipo de relatorio de auditoria invalido');
    }
    ...
    try {
      ...
      if (err.code === 'permission-denied') {
        setError('Sem permissao para enviar relatorios de auditoria');
      } else {
        setError('Erro ao fazer upload do relatorio de auditoria');
      }
      throw err;
    } finally {
      setLoading(false);
...
export default useAuditoriasRelatorios;
