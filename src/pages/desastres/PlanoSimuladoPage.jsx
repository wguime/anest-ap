...
export default function PlanoSimuladoPage({ onNavigate }) {
  ...
  const handleDocumentoClick = (doc) => {
    onNavigate('documento-detalhe', { documentoId: doc.id, returnTo: 'planoSimulado' });
  };

  return (
    <div className="min-h-dvh bg-background pb-24">
      <PageHeader title="Simulados de Emergencia" onBack={() => onNavigate('desastres')} />

      <div className="px-4 sm:px-5 py-4 space-y-4">
        <SectionCard title="Relatorios de Simulado" subtitle={`${documentos.length} documento${documentos.length !== 1 ? 's' : ''}`}>
          {documentos.length > 0 ? (
