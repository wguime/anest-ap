 ...
 * @param {number} breakpoint - Default 768 (alinhado com Tailwind lg:)
 */
...
export function CascadeCreator({ onNavigate, onComplete }) {
  ...
  const [currentStep, setCurrentStep] = useState(null); // Começa null, será setado depois
  ...
  // Limpar ao desmontar se estiver finalizado
  useEffect(() => {
    return () => {
      if (currentStep === 'done') {
  ...
  }, [createdEntities, linkCursoToTrilha, linkModuloToCurso, linkAulaToModulo, onComplete]);
  ...
  // Layout desktop: formulário + resumo lateral (mobile-first)
  return (
