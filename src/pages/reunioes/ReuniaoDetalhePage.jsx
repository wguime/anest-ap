    ...
    <nav className="fixed top-0 left-0 right-0 z-50 bg-card border-b border-border shadow-sm">
      <div className="px-4 sm:px-5 py-3 flex items-center justify-between">
        <div className="min-w-[70px]">
          <button type="button" onClick={() => onNavigate('reunioes')} className="flex items-center gap-1 text-primary hover:opacity-70 transition-opacity">
            <ChevronLeft className="w-5 h-5" /><span className="text-sm font-medium">Voltar</span>
          </button>
        </div>
        <h1 className="text-base font-semibold text-foreground truncate text-center flex-1 mx-2">Detalhes da Reunião</h1>
        <div className="min-w-[70px] flex justify-end">
          {availableActions.length > 0 && (
            <DropdownMenu>
              <DropdownTrigger asChild>
                <button type="button" aria-label="Mais ações da reunião" className="p-2 rounded-xl hover:bg-secondary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary">
              ...
              <DropdownContent align="end" minWidth={280}>
                {availableActions.map((act, i) => (
                  <DropdownItem key={i} onClick={act.action} icon={act.icon ? <act.icon className="w-4 h-4" /> : undefined} className={act.primary ? 'font-semibold text-primary' : ''}>{act.label}</DropdownItem>
                ))}
              </DropdownContent>
            </DropdownMenu>
          )}
