...
- **Vê e gere** (05/09, decisão do dono): SÓ responsável opt-in do tipo — admin não marcado NÃO vê, não gere, não baixa anexo (migration `20260905000000` derrubou `inc_*_admin`; storage e `incident_notification_settings` idem). Admin entra na seção Incidentes do Centro de Gestão apenas para gerir a aba Responsáveis. `rpc_anonimizar_incidente` exige `is_admin()`; retenção só por job. Firestore `incidentes`/`denuncias` travados. Original do dia anterior — admin (`admin_users`) OU responsável opt-in do tipo (`incident_notification_settings.receber_denuncias`/`receber_incidentes`, perfil ativo) — policies `inc_select_responsavel`/`inc_update_responsavel` e a de SELECT do bucket `incidentes-anexos`, todas via `is_incident_responsible(tipo)` (migration `20260904210000`). O que se marca no Centro de Gestão vale no banco. Sem INSERT direto (`inc_insert_auth` caiu): só `rpc_submit_incidente`.
- **Avisado**: SÓ quem está marcado como responsável do tipo com "avisar no app", admin ou não, sem fallback — único escritor é o trigger `notify_responsaveis_on_incidente` (grave/crítico e Never Event saem como urgente lá). Ele grava a notificação in-app **e dispara o push** (`vault` `edge_fn_service_role` + `net.http_post` → `send-fcm-push`, que aceita a service_role key do projeto desde 05/09). ⚠️ Ao mover o aviso para o banco em 04/09 o push sumiu por um dia: ele vinha de carona no `createNotificationBatch` do cliente. Quem mexer nesse trigger mantém os DOIS canais. O cliente NÃO cria mais a notificação de relato novo; mudança de status usa `getResponsaveisOptIn`. Se ninguém estiver marcado, ninguém é avisado — a rede é o e-mail institucional.
