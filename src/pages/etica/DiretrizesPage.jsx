...
export default function DiretrizesPage({ onNavigate, user }) {
  ...
  useEffect(() => {
    loadDocumento('diretrizes');
  }, [loadDocumento]);

  // Handler de upload
  const handleUpload = async (file, metadata) => {
    await uploadDocumento('diretrizes', file, metadata, user);
  };

  // Handler de exclusao
  const handleDelete = async () => {
    if (!documento) return;
    await deleteDocumento('diretrizes', documento.id, documento.storagePath);
    setShowDeleteModal(false);
  };

  return (
    <div className="min-h-dvh bg-background pb-24">
      <PageHeader title="Diretrizes Institucionais" onBack={() => onNavigate('eticaBioetica')} />
