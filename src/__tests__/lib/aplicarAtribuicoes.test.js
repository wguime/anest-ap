...
describe('salasDoHospital', () => {
  it('HRO: traz TODAS as salas da base, não só as que já têm caso', () => {
    ...
    for (const s of ['Sala 1', 'Sala 3', 'Sala 5', 'Sala 7', 'IOSC', 'Hospital de Olhos']) {
  ...
  // A mesma sala do HRO aparece em produção como "Sala 5", "Sala 5 - Emergência"
  // e "Bloco A - Sala 5". Oferecer duas delas faria a MESMA sala virar dois blocos
  // no quadro — o motivo de o dedupe ser por identidade, não por texto.
  it('HRO: grafia antiga e canônica da mesma sala não aparecem juntas', () => {
    const out = salasDoHospital('hro', [{ sala: 'Sala 5 - Emergência' }])
    expect(out).toContain('Sala 5 - Emergência')
    expect(out).not.toContain('Sala 5')
    expect(out).toContain('Sala 3') // as demais seguem canônicas
  })
