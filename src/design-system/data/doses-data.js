...
export function calculatePediatricDose(drugId, weightKg) {
  ...
  const concentration = drug.dilution === 'Sem diluir'
    // ⚠️ aceita vírgula: os dados são exibidos na tela e escritos em português.
    ? numeroFlexivel(drug.presentation)
    : numeroFlexivel(drug.dilution.match(/(\d+[.,]?\d*)\s*(mg|mcg|mEq)\/mL/)?.[1] || drug.dose);
