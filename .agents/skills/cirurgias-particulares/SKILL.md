...
### status
Leitura (sempre via `node .agents/skills/escala-cirurgica/scripts/query-ro.mjs "<SQL>"` — SELECT-only):
```sql
...
6. **Registrar** — docs/cirurgias-particulares.md + `.claude/rules/faturamento-particulares.md`
   (a regra do módulo mora lá; o AGENTS.md só aponta) + memória do agente (arquivo feedback_*)
