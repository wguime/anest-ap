...
**Navegação (App.jsx):** cases `cateteresPeridural` / `novoCateter` / `cateterDetalhe`. Permissões em `PAGE_TO_CARD`: listagem usa o card `cateter_peridural`; subcards `cp_novo` (novoCateter) e `cp_listagem` (cateterDetalhe). A bottom nav não tem badge (visual travado — AGENTS.md).
