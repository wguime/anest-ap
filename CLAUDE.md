...
| Codificação Anestésica | card em MenuPage → `src/pages/codificacao-anestesica/` | Supabase `unimed_tuss_codigos` (read-only, ~5.4k TUSS HM+SADT) + lib pura `src/lib/codificacaoAnest{,Rules}.js` + `src/data/codigosAnestesia.js` (grupo 31602 curado) | Calculadora de guia estilo Volan: autocomplete (digita **código OU nome** → sugestões ao vivo via RPC `search_unimed_tuss` — **acento-insensível + multi-palavra**, migration `20260622140000` + extensão `unaccent`; com **favoritos** em localStorage `anest-cod-fav`, **sem ícone de lupa**) → cada linha com **quantidade** + **% auto-preenchido** (maior valor=100%/demais=50% via `sugerirPercentuais`; reaplica ao add/remove, respeita edição manual `manual:true`) + badge **DropdownMenu** com `OPCOES_PERCENTUAL` (10/30/40/50-mesma via/70-outra via/100-principal) + valor cirurgião/anestesista. **UTM fixa R$ 1,73** (Chapecó, sem seletor) = stored×(1,73/1,17). **Acomodação** (`Select`, `ACOMODACOES`): **Apartamento=2×**, Enfermaria/Ambulatório/One Day=1× → `calcularGuia(opts.acomodacaoMult)`. Valor adicional + Eletiva/Emergência + totais. Anestesia zera (porte 0/SADT) → recomenda **31602355**/**31602347** ou "Anestesia para exames de…" (mapa `RECOMENDACAO_EXAME` por natureza) + justificativa. Consulta = referência 31602 por situação (valores em 1,73) para "autorizado sem valor → qual código"; cada item é **accordion** (título+valor; clique abre a explicação "quando usar"). ⚠ **percentuais redutores NÃO constam do referencial** (NOTA oficial) — auto/badge são estimativa ajustável. Sem gate. Seed: `scripts/extract-tuss-from-xlsx.mjs` → `scripts/seed-unimed-tuss.mjs`. Ref. `docs/codigos-anestesia.md` |
