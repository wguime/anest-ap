...
| Codificação Anestésica | card em MenuPage → `src/pages/codificacao-anestesica/` | Supabase `unimed_tuss_codigos` (read-only, ~5.4k TUSS HM+SADT) + lib pura `src/lib/codificacaoAnest{,Rules}.js` + `src/data/codigosAnestesia.js` (grupo 31602 curado) | Calculadora de guia estilo Volan: autocomplete (digita → sugestões ao vivo via `searchCodigos`, com **favoritos** em localStorage `anest-cod-fav`) → cada linha com **quantidade** + **% editável (default 100, modelo Volan)** + porte + valor cirurgião/anestesista; **valor adicional** + toggle **Eletiva/Emergência**; totais. Quando anestesia zera (porte 0/SADT) recomenda **31602355** (imperativo clínico)/**31602347** (sem porte) ou "Anestesia para exames de…" + justificativa. ⚠ **percentuais redutores NÃO constam do referencial** (NOTA oficial); por isso default 100% editável por linha (não há ladder automático) + `sugerirPercentuais` (não-oficial: maior 100%/demais 50%). Sem gate. Tabelas Intercâmbio (1,17) e Chapecó (1,73=derivada ×1,73/1,17). Seed: `scripts/extract-tuss-from-xlsx.mjs` → `scripts/seed-unimed-tuss.mjs`. Ref. `docs/codigos-anestesia.md` |
